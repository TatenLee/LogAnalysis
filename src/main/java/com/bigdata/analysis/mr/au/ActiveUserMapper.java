...
/**
 * @Author Taten
 * @Description 统计活跃的用户，launch时间中uuid的去重个数
 **/
public class ActiveUserMapper extends TableMapper<StatsUserDimension, TimeOutputValue> {
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        k.getStatsCommonDimension().getKpiDimension().setKpiName(KpiType.ACTIVE_USER.kpiName);
        ...
        for (PlatformDimension platformDimension : platformDimensionList) {
            ...
            // 循环浏览器维度输出
            k.getStatsCommonDimension().getKpiDimension().setKpiName(KpiType.BROWSER_ACTIVE_USER.kpiName);
