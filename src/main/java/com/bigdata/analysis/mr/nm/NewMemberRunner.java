...
import com.bigdata.analysis.mr.nu.NewUserMapper;
import com.bigdata.analysis.mr.nu.NewUserReducer;
...
public class NewMemberRunner implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        // 设置mapper属性
        TableMapReduceUtil.initTableMapperJob(this.buildList(job), NewMemberMapper.class, StatsUserDimension.class, TimeOutputValue.class, job, true);
        ...
        if (job.waitForCompletion(true)) {
            this.computeTotalNewMembers(job);  // 计算新增总会员
            return 0;
        } else {
            return 1;
        }
    ...
    /**
     * 计算新增总会员
     * 获取运行当天的新增会员，再获取运行日期前一天的新增总会员，然后将相同维度的新增会员和总会员相加即可
     *
     * @param job
     */
    private void computeTotalNewMembers(Job job) {
        ...
        try {
            ...
            // 获取今天的新增用户
            if (nowDimensionId > 0) {
                ps = conn.prepareStatement(conf.get(GlobalConstants.PREFIX_TOTAL + "new_total_members"));
                // 赋值
                ps.setInt(1, nowDimensionId);
                rs = ps.executeQuery();
                while (rs.next()) {
                    int platformId = rs.getInt("platform_dimension_id");
                    int new_members = rs.getInt("new_members");
                    // 存储
                    if (map.containsKey(platformId + "")) {
                        new_members += map.get(platformId + "");
                    }
                    // 覆盖
                    map.put(platformId + "", new_members);
                }
            }

            // 将map中数据进行更新
            ps = conn.prepareStatement(conf.get(GlobalConstants.PREFIX_TOTAL + "new_update_members"));
