...
load data inpath '/ods/month=05/day=28' into table logs partition(month=05,day=28);


1、创建事件的基础维度类，不需要使用集合维度对象

2、创建获取维度id的udf函数，并测试
create function date_convert as 'com.bigdata.analysis.hive.DateDimensionUdf' using jar "hdfs://hadoop01:9000//logs/udf/jars/LogAnalysis-1.0-SNAPSHOT.jar";
create function platform_convert as 'com.bigdata.analysis.hive.PlatformDimensionUdf' using jar "hdfs://hadoop01:9000//logs/udf/jars/LogAnalysis-1.0-SNAPSHOT.jar";
create function event_convert as 'com.bigdata.analysis.hive.EventDimensionUdf' using jar "hdfs://hadoop01:9000//logs/udf/jars/LogAnalysis-1.0-SNAPSHOT.jar";
...
with tmp as(
select
from_unixtime(cast(1.s_time/1000 as bigint), "yyyy-MM-dd") as dt,
1.pl as pl,
1.ca as ca,
1.ac as ac,
count(*) ct
...
where month = 5
and day = 28
group by from_unixtime(cats(1.s_time/1000 as bigint), "yyyy-MM-dd"), pl, ca, ac;
)
select dt as dt, pl as pl, ca as ca, ac as ac
from

=================思路=======================：
1534493474123	website	null	null
1534493474456	java_server	null	null
...
扩维：
2018-08-17 website null	null 1
2018-08-17 website null	all 1

2018-08-17 java_server	null	null 1
2018-08-17 java_server	null	all 1


2018-08-17 all	null	all 2
2018-08-17 all	null	null 2
